...
func MapToQuery(m types.SQLMap, noargs ...bool) (string, []interface{}) {
	...
	for k, v := range m.Map {
		...
		if m.ComparisonOperator == enum.In || m.ComparisonOperator == enum.NotIn {
...
// MapToSQuery converts an types.SQLMap to an SQL sum query string
// suitable for use with bun.NewRaw
func MapToSQuery(m types.SQLMap) string {
	var query string
	i := 0
	for k := range m.Map {
		query += `SUM(` + k + `)`
...
// MapToIQuery converts an types.SQLMap to an SQL insert query string and a slice of arguments
// suitable for use with bun.NewRaw
func MapsToIQuery(m types.SQLMaps) (string, []interface{}) {
	var query string
	var subquery string
	var args []interface{}
	...
	if len(m.Conflict) > 0 && m.SMap.Map != nil {
...
// MapToOQuery converts an types.SQLMap to an SQL order by query string
// suitable for use with bun.NewRaw
func MapsToOQuery(m types.SQLMaps) string {
	var query string
	var i int
